...
ELEV_BIN_DEG = 5       # elevation bin width for CN0 model
ELEV_BINS    = np.arange(0, 95, ELEV_BIN_DEG)   # 0,5,10,...,90
...
ap.add_argument('--cn0_thresh',type=float, default=6.0,
                help='C/N0 deficit threshold in dB-Hz (default 6)')
...
dd_map = {}   # (rinex_t_key, sat_id) -> nlos_dd (int)
...
# ── pass 1: collect all (elev, cn0) pairs to build model ─────────────
...
print(f'  {len(all_cn0)} GPS L1 C/N0 observations collected')
print(f'  C/N0 range: {all_cn0.min():.1f} – {all_cn0.max():.1f} dB-Hz  '
      f'median={np.median(all_cn0):.1f}')

# ── build binned-median elevation model ───────────────────────────────
...
for b in ELEV_BINS:
    ...
    print(f'  elev {b:2d}–{b+ELEV_BIN_DEG:2d}°  n={mask.sum():4d}  '
...
# confusion: DD vs LiDAR (reference from Step 6b)
...
print(f'\n  CN0 ∩ DD agreement: {both} observations  '
      f'({100*both/n_total:.1f}% of all, '
...
# Fig 1: C/N0 vs elevation — scatter colored by LiDAR label
...
if len(bin_centers):
    ...
    ax.fill_between(bin_centers, bin_p25, bin_p75, alpha=0.15, color='k',
                    label='IQR band')
ax.axhline(np.median(cn0s) - args.cn0_thresh, color='orange', ls='--', lw=1.5,
           label=f'Approx deficit threshold ({args.cn0_thresh} dB)')
ax.set_xlabel('Elevation (°)'); ax.set_ylabel('C/N0 (dB-Hz)')
...
# Fig 2: C/N0 deficit histogram (LOS vs LiDAR-NLOS)
...
# Fig 4: Venn-like overlap bar (3-way)
...
labels_v = ['CN0 only', 'DD only', 'LiDAR only',
            'CN0∩DD', 'CN0∩LiDAR', 'DD∩LiDAR', 'All 3']
...
# Fig 5: deficit vs DD residual scatter (GPS only, colored by LiDAR)
dd_resids_all = []
deficits_all  = []
lidar_labels  = []
try:
    ...
    for row in out_rows:
        ...
        if key in dd_full:
            dd_resids_all.append(dd_full[key])
            deficits_all.append(float(row['cn0_deficit']))
            lidar_labels.append(row['nlos_lidar'])
    dd_resids_all = np.array(dd_resids_all)
    deficits_all  = np.array(deficits_all)
    lidar_labels  = np.array(lidar_labels)
    fig, ax = plt.subplots(figsize=(8, 5))
    ax.scatter(dd_resids_all[lidar_labels==0], deficits_all[lidar_labels==0],
               s=4, alpha=0.3, color='#2196F3', label='LiDAR LOS')
    ax.scatter(dd_resids_all[lidar_labels==1], deficits_all[lidar_labels==1],
               s=4, alpha=0.5, color='#F44336', label='LiDAR NLOS')
    ax.axhline(args.cn0_thresh, color='orange', ls='--', lw=1.2,
               label=f'CN0 thresh {args.cn0_thresh}')
...
<p>UrbanNav HK Medium-Urban-1 &nbsp;|&nbsp; {n_total} GPS L1 obs &nbsp;|&nbsp;
Elevation cutoff: {args.min_elev}&deg; &nbsp;|&nbsp; Deficit threshold: {args.cn0_thresh}&nbsp;dB-Hz</p>

<div class="note">
<b>Method:</b> For each GPS satellite, compute C/N0 deficit = expected_C/N0(elev) − actual_C/N0.
Expected C/N0 is the binned median across all observations in 5° elevation bands (data-driven model).
Deficit &gt; {args.cn0_thresh}&nbsp;dB-Hz is flagged as NLOS.
</div>
...
<h2>3. Precision / Recall / F1 Comparison</h2>
<img src="data:image/png;base64,{imgs['prec_rec']}">

<h2>4. 3-Way NLOS Overlap</h2>
<img src="data:image/png;base64,{imgs['venn']}">

{'<h2>5. Feature Space: DD residual vs C/N0 deficit</h2><img src="data:image/png;base64,' + imgs['feature_space'] + '">' if 'feature_space' in imgs else ''}
